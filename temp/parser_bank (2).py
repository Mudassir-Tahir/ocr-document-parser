...
def parse_bank_statement(text):
    ...
    # --- General Info ---
    opening_balance = extract_match(r"(opening balance)\s*[:\-]?\s*([0-9.,]+)", text, 2)
    closing_balance = extract_match(r"(closing balance)\s*[:\-]?\s*([0-9.,]+)", text, 2)
    statement_date = extract_match(r"(statement date)\s*[:\-]?\s*(\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4})", text, 2)
    period_covered = extract_match(r"(period covered|date range)\s*[:\-]?\s*(.+?)(?=\n|$)", text, 2)
